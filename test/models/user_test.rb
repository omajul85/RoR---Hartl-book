...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid addresses" do
  	...
  	valid_addresses.each do |v|
  		...
  		# The line below identifies the the address causing the test to fail
  		assert @user.valid?, "#{v.inspect} should be valid" 
  	end
  end

  test "email validation should reject invalid addresses" do
  	invalid_addresses = %w(foo@bar..com user@example,com USER_at_foo.COM A_US-ER@foo.bar@org foo@bar+baz.cn)
